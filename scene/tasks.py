...
# Each task carries a default ``instruction`` -- the language instruction that
# pre-fills the data-collection input when the task is selected (editable there).
TASKS = {
    # Just the base scene, nothing to manipulate.
    "empty": dict(base="fr3_with_gripper", instruction="", objects=[]),

    # Single cube to pick up. Sized well within the 0.08 m gripper opening.
    "pick_cube": dict(base="fr3_with_gripper",
                      instruction="pick up the red cube", objects=[
    ...
    # overlaps them).
    "stack_blocks": dict(base="fr3_with_gripper",
                         instruction="stack the red block on the blue block", objects=[
...
def task_instruction(name):
    """Default language instruction for a task (empty string if none/unknown).
    Pre-fills the collection instruction input when the task is selected."""
    return TASKS.get(name, {}).get("instruction", "")
