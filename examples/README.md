...
across the set (`overlay.TARGET_RGBA` / `overlay.ACTUAL_RGBA`). Viewer-overlay
helpers (`add_marker`, `add_frame`, the colors) live in the `overlay/` package,
shared with `teleop`. The pose/command conventions — `pose_to_vec` /
